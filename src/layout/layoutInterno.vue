  ...
  methods: {
    ...
    async gerarRelatorioRegistros () {
      ...
      const res = await this.gerarRelatorio({
        ...
        dados: this.registrosNotificacoes && this.registrosNotificacoes.length ? this.registrosNotificacoes.map(item => colunas.map(coluna => coluna.value === 'razao_social' && (item[coluna.value] || '').length > 50 ? item[coluna.value].slice(0, 50) + '[...]' : item[coluna.value] || '')) : null
      })
